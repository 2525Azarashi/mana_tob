...
const TermsPage: React.FC<Props> = ({ setCurrentPage }) => (
  <PageShell
    ...
    lead={`本規約は、「マナトビ」の3つの活動のうち「マナトビアプリ」（以下「当団体」）が運営するウェブサイトおよび提供する教材・サービス（以下あわせて「本サービス」）の利用条件を定めるものです。本サービスをご利用いただく際は、本規約に同意いただいたものとみなします。`}
    setCurrentPage={setCurrentPage}
  >
    <div className="max-w-3xl">
      <NoteBox title="本規約の適用範囲と運営主体">
        <p>
          本規約は、マナトビ公式サイト（{ORG.siteUrl.replace('https://', '')}）、
          Chem-Basis（{ORG.chemBasisUrl.replace('https://', '')}）、
          および当団体が公開する教材（PDF・Web教材等）に適用されます。
        </p>
        <p>
          「マナトビ」は、3つの活動（マナトビアプリ、学生学修コミュニティ「まなとび」、
          音楽活動「まなとび」）の総称であり、それぞれ資金・会計・運営を
          独立して行っています。本規約にいう「当団体」は、本サービスを運営する
          「マナトビアプリ」のみを指し、他の2つの活動は本規約の当事者となりません。
      ...
      <P>
        ...
        学生が主体となって運営する当団体（マナトビアプリ）が無償で提供するものです。
      </P>
      <P>
