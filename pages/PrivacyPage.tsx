...
const PrivacyPage: React.FC<Props> = ({ setCurrentPage }) => (
  <PageShell
    ...
    lead={`当サイトの運営主体である「マナトビアプリ」（以下「当団体」）は、当団体が運営するウェブサイトおよびサービス（以下あわせて「当サイト」）における個人情報およびそれに準ずる情報の取り扱いについて、以下のとおり定めます。`}
    setCurrentPage={setCurrentPage}
  >
    <div className="max-w-3xl">
      <NoteBox title="このポリシーの適用範囲と運営主体">
        <p>
          本ポリシーは、マナトビ公式サイト（{ORG.siteUrl.replace('https://', '')}）
          および当団体が開発・運営する Chem-Basis（
          {ORG.chemBasisUrl.replace('https://', '')}）に適用されます。
        </p>
        <p>
          なお「マナトビ」は、3つの活動（マナトビアプリ、学生学修コミュニティ「まなとび」、
          音楽活動「まなとび」）の総称です。当サイトおよび Chem-Basis の運営主体は
          このうち「マナトビアプリ」であり、本ポリシーにもとづく責任は同活動が負います。
      ...
      <P>
        一問一答や演習機能の進捗・解答状況などは、ご利用の端末のブラウザ内
